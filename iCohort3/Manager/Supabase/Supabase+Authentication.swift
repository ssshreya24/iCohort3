...
extension SupabaseManager {
    enum LoginOTPUserRole: String, Codable {
        case admin
        case student
        case mentor
    }
    ...
    func countApprovedMentorsByDomain(instituteDomain: String) async throws -> Int {
        ...
        let institute = try await getInstitute(byDomain: instituteDomain)
        let instituteName = institute?.name ?? ""

        struct MentorProfileCountRow: Decodable {
            let person_id: String
        }

        let mentors: [MentorProfileCountRow]
        if !instituteName.isEmpty {
            mentors = try await client
                .from("mentor_profiles")
                .select("person_id")
