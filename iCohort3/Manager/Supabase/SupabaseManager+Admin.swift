...
extension SupabaseManager {
    ...
    private struct StudentTeamDomainRow: Decodable, Sendable {
    ...
    func fetchTeamsCount(forInstituteDomain instituteDomain: String) async throws -> Int {
        let rows = try await fetchAdminTeamRows(forInstituteDomain: instituteDomain)
        return rows.count
    }
    ...
    private func fetchStudentDomains(personIds: [String]) async throws -> [String: String] {
        guard !personIds.isEmpty else { return [:] }

        let uniqueIds = Array(Set(personIds))
        let rows: [StudentTeamDomainRow] = try await client
            .from("student_profiles")
            .select("person_id, institute_domain")
            .in("person_id", values: uniqueIds)
            .execute()
            .value

        return rows.reduce(into: [String: String]()) { partialResult, row in
            if let domain = row.institute_domain?.trimmingCharacters(in: .whitespacesAndNewlines),
               !domain.isEmpty {
                partialResult[row.person_id] = domain
            }
        }
    }

    func fetchAdminTeamRows(forInstituteDomain instituteDomain: String) async throws -> [AdminTeamRow] {
        let rows = try await fetchAdminTeamRows()
        let creatorIds = rows.map(\.created_by_id)
        let domainsByPersonId = try await fetchStudentDomains(personIds: creatorIds)

        let filteredRows = rows.filter { row in
            domainsByPersonId[row.created_by_id] == instituteDomain
        }

        print("✅ Filtered \(filteredRows.count) team rows for domain \(instituteDomain)")
        return filteredRows
    }
    ...
    func fetchAllTeamsWithDetails(forInstituteDomain instituteDomain: String) async throws -> [TeamWithDetails] {
        print("🔍 fetchAllTeamsWithDetails for domain \(instituteDomain)...")
        let rows = try await fetchAdminTeamRows(forInstituteDomain: instituteDomain)
    ...
    func fetchAdminTeamRowForUser(userId: String) async throws -> AdminTeamRow? {
        let cols = "id, team_number, created_by_id, created_by_name, member2_id, member2_name, member3_id, member3_name, mentor_id, mentor_name, problem_statement, status, created_at"
    ...
    func fetchAdminTeamRow(teamId: String) async throws -> AdminTeamRow? {
        let cols = "id, team_number, created_by_id, created_by_name, member2_id, member2_name, member3_id, member3_name, mentor_id, mentor_name, problem_statement, status, created_at"
